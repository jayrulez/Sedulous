...
namespace Win32.Graphics.Direct3D12
{
	...
	{
		public this(D3D12_DESCRIPTOR_RANGE_TYPE rangeType, uint32 numDescriptors, int32 baseShaderRegister, int32 registerSpace = 0, uint32 offsetInDescriptorsFromTableStart = uint32(-1))
		{
			RangeType = rangeType;
	...
	{
		public this(D3D12_RESOURCE_DIMENSION dimension, uint64 alignment, uint64 width, uint32 height, uint16 depthOrArraySize, uint16 mipLevels, DXGI_FORMAT format, int32 sampleCount, int32 sampleQuality, D3D12_TEXTURE_LAYOUT layout, D3D12_RESOURCE_FLAGS flags)
		{
			Dimension = dimension;
			Alignment = alignment;
			Width = width;
		...
		public static D3D12_RESOURCE_DESC Texture2D(DXGI_FORMAT format, uint64 width, uint32 height, uint16 arraySize = 1, uint16 mipLevels = 0, int32 sampleCount = 1, int32 sampleQuality = 0, D3D12_RESOURCE_FLAGS flags = .D3D12_RESOURCE_FLAG_NONE, D3D12_TEXTURE_LAYOUT layout = .D3D12_TEXTURE_LAYOUT_UNKNOWN, uint64 alignment = 0uL)
		{
		...
		public static D3D12_RESOURCE_DESC Texture3D(DXGI_FORMAT format, uint64 width, uint32 height, uint16 depth, uint16 mipLevels = 0, D3D12_RESOURCE_FLAGS flags = .D3D12_RESOURCE_FLAG_NONE, D3D12_TEXTURE_LAYOUT layout = .D3D12_TEXTURE_LAYOUT_UNKNOWN, uint64 alignment = 0uL)
		{
	...
	{
		public this(D3D12_HEAP_TYPE Type, D3D12_CPU_PAGE_PROPERTY CPUPageProperty = .D3D12_CPU_PAGE_PROPERTY_UNKNOWN, D3D12_MEMORY_POOL MemoryPoolPreference = .D3D12_MEMORY_POOL_UNKNOWN, uint32 CreationNodeMask = 0, uint32 VisibleNodeMask = 0)
		{
			this.Type = Type;
			this.CPUPageProperty = CPUPageProperty;
			this.MemoryPoolPreference = MemoryPoolPreference;
			this.CreationNodeMask = CreationNodeMask;
			this.VisibleNodeMask = VisibleNodeMask;
		}
	}
}
