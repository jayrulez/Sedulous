...
namespace Sedulous.Foundation.Mathematics
{
	...
	/// </summary>
	//[Serializable]
	public struct Color : IEquatable<Color>, IEquatable, IHashable, IInterpolatable<Color>
	{
		/// <summary>
		...
		static this()
		{
			var properties = typeof(Color).GetFields(BindingFlags.Public | BindingFlags.Static | .GetProperty);
			for (var property in properties)
			{
				if (property.GetType() == typeof(Color))
				{
					var name = property.Name;
					var value = property.GetValue(null);
					NamedColorRegistry[name] = value.Value.Get<Color>();
				}
			}
		}
		...
		public this(float r, float g, float b)
		{
			...
			this.packedValue = (uint32)((rbyte) | ((uint32)gbyte << 8) | ((uint32)bbyte << 16) | ((uint32)255 << 24));
		}
		...
		/// <inheritdoc/>
		public override void ToString(String outStr) => outStr.Append(scope $"#{A:x2}{R:x2}{G:x2}{B:x2}");
		...
		private static readonly Dictionary<StringView, Color> NamedColorRegistry =
			new Dictionary<StringView, Color>() ~ delete _;
