...
namespace Sedulous.Foundation.Mathematics
{
    ...
    public class QuaternionCurveLinearSampler : ICurveSampler<Quaternion, CurveKey<Quaternion>>
    {
        ...
        /// <inheritdoc/>
        public void CreateTemporaryValue(int elementCount, out Quaternion value) => value = default;

        /// <inheritdoc/>
        public void ReleaseTemporaryValue(in Quaternion value) { }

        /// <inheritdoc/>
        public Quaternion InterpolateKeyframes(CurveKey<Quaternion> key1, CurveKey<Quaternion> key2, float t, Quaternion offset, in Quaternion existing)
        {
            var key1Value = key1.Value;
            var key2Value = key2.Value;

            Quaternion.Slerp(ref key1Value, ref key2Value, t, var result);
            return offset + result;
        }

        /// <inheritdoc/>
        public Quaternion CalculateLinearExtension(CurveKey<Quaternion> key, float position, CurvePositionType positionType, in Quaternion existing) =>
            key.Value;
        ...
        /// </summary>
        public static QuaternionCurveLinearSampler Instance { get; } = new QuaternionCurveLinearSampler() ~ delete _;
    }
}
