...
namespace Sedulous.Foundation.Mathematics
{
        ...
        where TKey : CurveKey<TValue>
    {
        ...
        public this(CurveLoopType preLoop, CurveLoopType postLoop, ICurveSampler<TValue, TKey> sampler, Span<TKey> keys)
        ...
        /// <inheritdoc/>
        public override TValue Evaluate(float position, in TValue existing)
        {
            var keys = Keys;
            ...
            if (position < keyFirst.Position)
                return EvaluateOutside(PreLoop, keyFirst, position, CurvePositionType.BeforeCurve, existing);

            if (position > keyLast.Position)
                return EvaluateOutside(PostLoop, keyLast, position, CurvePositionType.AfterCurve, existing);

            return EvaluateInside(position, default(TValue), existing);
        }

        /// <inheritdoc/>
        public override float StartPosition { get; protected set; }

        /// <inheritdoc/>
        public override float EndPosition { get; protected set; }
        ...
        /// </summary>
        public bool IsConstant => Keys.IsConstant;
        ...
        /// </summary>
        private int32 GetCycleIndex(float position)
        {
            var cycle = (position - keyFirst.Position) / Length;
            return (cycle < 0) ? (int32)cycle - 1 : (int32)cycle;
        }

        /// <summary>
        /// Evaluates a position inside of the curve.
        /// </summary>
        private TValue EvaluateInside(float position, TValue offset, in TValue existing)
        {
            FindKeyRecordsAtCurvePosition(position, var record1, var record2);

            var key1 = record1.Key;
            var key2 = record2.Key;

            var factor = (position - key1.Position) / (key2.Position - key1.Position);
            var result = (record1.SamplerOverride != null ? record1.SamplerOverride : Sampler).InterpolateKeyframes(key1, key2, factor, offset, existing);
        ...
        /// </summary>
        private TValue EvaluateOutside(CurveLoopType loop, TKey loopKey, float position, CurvePositionType positionType, in TValue existing)
        {
			var position;
        ...
        /// </summary>
        private void FindKeyRecordsAtCurvePosition(float position, out CurveKeyRecord<TValue, TKey> key1, out CurveKeyRecord<TValue, TKey> key2)
        {
            var keys = Keys;
            key1 = keys[0];
