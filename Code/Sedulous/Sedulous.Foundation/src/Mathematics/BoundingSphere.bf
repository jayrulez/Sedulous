...
namespace Sedulous.Foundation.Mathematics
{
    ...
    /// </summary>
    //[Serializable]
    public struct BoundingSphere : IEquatable<BoundingSphere>, IEquatable, IHashable
    {
        /// <summary>
        ...
        public static BoundingSphere CreateMerged(BoundingSphere original, BoundingSphere additional)
        {
			var original;
			var additional;
            CreateMerged(ref original, ref additional, var result);
            return result;
        }
        ...
        {
            Vector3.Subtract(ref additional.Center, ref original.Center, var offset);
        ...
        public static BoundingSphere CreateFromPoints(IEnumerator<Vector3> points)
        {
        ...
        public static void CreateFromPoints(IEnumerator<Vector3> points, out BoundingSphere result)
        {
            ...
            if (current case .Err)
                Runtime.FatalError(nameof(points));

            var minX = current.Get();
            var maxX = current.Get();
            var minY = current.Get();
            var maxY = current.Get();
            var minZ = current.Get();
            var maxZ = current.Get();
            ...
            Vector3.Distance(ref minX, ref maxX, var diameterX);
            Vector3.Distance(ref minY, ref maxY, var diameterY);
            Vector3.Distance(ref minZ, ref maxZ, var diameterZ);
        ...
        public static void CreateFromPoints(Span<Vector3> points, out BoundingSphere result)
        {
            ...
            if (points.Length == 0)
                Runtime.FatalError(scope $"Argument error: '{nameof(points)}'.");
            ...
            for (int i = 0; i < points.Length; i++)
            {
                ...
                Vector3.Subtract(ref point, ref center, var pointRelativeToCenter);
        ...
        /// <inheritdoc/>
        public override void ToString(String outStr) => outStr.Append(scope $"{{Center:{Center} Radius:{Radius}}}");
        ...
        public ContainmentType Contains(Vector3 point)
        {
			var point;
			var center = Center;
            Vector3.DistanceSquared(ref point, ref center, var distanceSquared);
        ...
        {
			var center = Center;
            Vector3.DistanceSquared(ref center, ref sphere.Center, var distanceSquared);
        ...
        public bool Intersects(BoundingFrustum frustum)
        {
			...
			var self = this;
            frustum.Intersects(ref self, var result);
            return result;
        }
        ...
        public void Intersects(BoundingFrustum frustum, out bool result)
        {
			...
			var self = this;
            frustum.Intersects(ref self, out result);
        }
        ...
        {
			var sphere;
            Intersects(ref sphere, var result);
            return result;
        }
        ...
        {
			var self = this;
            @box.Intersects(ref self, out result);
        }
        ...
        {
			var plane;
            Intersects(ref plane, var result);
            return result;
        }
        ...
        {
			var center = Center;
            Vector3.Dot(ref plane.Normal, ref center, var distance);
            distance += plane.D;
        ...
        {
			var self = this;
            ray.Intersects(ref self, var result);
            return result;
        }
        ...
        {
			var self = this;
            ray.Intersects(ref self, out result);
        }
