...
public class ExpandingPool<T> : Pool<T>
{
	...
	public this(int32 capacity, int32 watermark, delegate T() allocator = null, delegate void(T) deallocator = null)
	{
		...
		for (var @interface in typeof(T).Interfaces)
		{
	...
	protected override void Dispose(bool disposing)
	{
		if (disposing && disposable)
		{
			for (var item in storage)
			{
				((IDisposable)(Object)item).Dispose();
			}
		}
	}
	...
	{
		var capacity = (int32)Math.Min(watermark, ((3 * storage.Count) / 2) + 1);
		ExpandStorage(capacity);
	}
	...
	private void ExpandStorage(int32 capacity)
	{
		...
		for (int i = oldCount; i < storage.Count; i++)
			storage[i] = allocator();
	}
