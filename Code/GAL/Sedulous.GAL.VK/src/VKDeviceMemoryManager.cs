	...
	internal class VKDeviceMemoryManager : IDisposable
	{
			...
			VkBuffer dedicatedBuffer)
		{
			...
			if (dedicated)
			{
				...
				{
					VkBufferMemoryRequirementsInfo2 requirementsInfo = VkBufferMemoryRequirementsInfo2() { sType = .VK_STRUCTURE_TYPE_BUFFER_MEMORY_REQUIREMENTS_INFO_2 };
					requirementsInfo.buffer = dedicatedBuffer;
					VkMemoryRequirements2 requirements = VkMemoryRequirements2() { sType = .VK_STRUCTURE_TYPE_MEMORY_REQUIREMENTS_2 };
			...
			using (_lock.Enter())
			{
				...
				{
					VkMemoryAllocateInfo allocateInfo = VkMemoryAllocateInfo() { sType = .VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO };
				...
				else
				{
					...
					bool result = allocator.Allocate(size, alignment, var ret);
					if (!result)
					{
						Runtime.GALError("Unable to allocate sufficient Vulkan memory.");
					}
		...
		private class ChunkAllocator : IDisposable
		{
			...
			public this(VkDevice device, uint32 memoryTypeIndex, bool persistentMapped)
			{
				...
				VkMemoryAllocateInfo memoryAI = VkMemoryAllocateInfo() { sType = .VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO };
		...
		{
			void* pData = null;
			VkResult result = vkMapMemory(_device, memoryBlock.DeviceMemory, memoryBlock.Offset, memoryBlock.Size, 0, &pData);
			CheckResult(result);
			return pData;
		}
	}
